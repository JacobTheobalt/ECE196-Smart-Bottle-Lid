...
float get_water_level()
{
  ...
  while(measurement < 100)
  {
    ...
    if ((!status) && (NewDataReady != 0)) {
      ...
      //store the measurement at index 'measurement' in water_levels array only if it is less than 32
      if(results.distance_mm > 32)
      {
        water_levels[measurement] = 0;
      }
      else
      {
        water_levels[measurement] = results.distance_mm;
      }
//      Serial.println(water_levels[measurement]);
      measurement += 1;
